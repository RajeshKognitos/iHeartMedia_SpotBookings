...
- **No data in the app on Vercel:** (1) Confirm Kognitos env vars are set for **Production** (or the environment you’re viewing). (2) Open the deployment URL and click **Refresh** on the dashboard to force a fresh fetch. (3) In the browser, open `https://your-app.vercel.app/api/runs?period=all` — if you see runs there, the problem is cache/origin; if you see an error or `{"runs":[]}`, fix Kognitos config first. (4) In Vercel → **Deployments** → your deployment → **Functions** / **Logs**, look for `[cache/refresh] /api/runs returned 503` (or 401/500) to see which upstream API is failing. The app now builds the correct origin from `x-forwarded-host` / `host` so cache refresh calls your real Vercel URL, and it no longer caches all-empty data so a bad first run doesn’t stick.
